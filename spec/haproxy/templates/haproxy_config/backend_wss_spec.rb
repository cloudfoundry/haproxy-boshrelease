...
describe 'config/haproxy.config backend http-routers-ws' do
  let(:haproxy_conf) do
  ...
  let(:backend_http_routers_ws) { haproxy_conf['backend http-routers-ws'] }
  ...
  context 'when ha_proxy.disable_backend_http2_websockets is configured' do
    ...
    it 'receives websocket traffic from http-in' do
      expect(frontend_http).to include('use_backend http-routers-ws if is_websocket')
    end
    ...
    context 'when https is enabled' do
      ...
      context 'when the port 4443 websocket frontend is enabled' do
        ...
        it 'receives websocket traffic from wss-in' do
          expect(frontend_wss_in).to include('use_backend http-routers-ws if is_websocket')
        end
      end
    end
  end
