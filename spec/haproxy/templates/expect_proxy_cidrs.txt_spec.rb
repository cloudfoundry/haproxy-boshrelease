...
describe 'config/expect_proxy_cidrs.txt' do
  ...
  context 'when ha_proxy.expect_proxy' do
    context 'when a list of cidrs is provided' do
      it 'has the correct contents' do
        expect(template.render({
          'ha_proxy' => {
            'expect_proxy' =>['10.5.6.7/27',
                              '2001:db8::/32']
          }
        })).to eq(<<~EXPECTED)
          # generated from expect_proxy_cidrs.txt.erb
          
          # BEGIN expect_proxy cidrs
          10.5.6.7/27
          2001:db8::/32
          # END expect_proxy cidrs
        EXPECTED
      end
    end
  end
    context 'when ha_proxy.expect_proxy is not provided' do
      it 'is empty' do
